...
export default function ComponentsTable(sim: any) {
  ...
  const getDoctorinfos = async () => {
    const res = await api.listDoctorinfo({ limit: 100, offset: 0 });
    setLoading(false);
    setDoctorinfos(res);
  ...
  for (var val of Doctorinfos) {
    if (val.licensenumber === sim.sim ||sim.sim === undefined || sim.sim === null) {
      p = p + 1
    }
      ...
      return (
        ...
        <TableContainer component={Paper}>
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {Doctorinfos === undefined
                ? null
                : Doctorinfos.filter(i => i.licensenumber === sim.sim || sim.sim === undefined || sim.sim === null).map((item: any) => (
                  <TableRow>
